...
redraw() {
  ...
  for (let i = 0; i < this._paipu.length(); i++) {
    ...
    let player = [];
    for (let l = 0; l < 4; l++) {
      let point = paipu.point[l];
      let defen = (paipu.defen[l] > 0 ? '+' : '') + paipu.defen[l];
      player[l] = `${paipu.player[l]}[${point} (${defen})]`;
    }
...
error(msg) {
  let error = $('.error', this._node).append($('<div></div>').text(msg)).fadeIn();
