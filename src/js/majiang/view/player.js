...
show_button() {
  $('.UI', this._root)
    .width($('.shoupai.main .shouli', this._root).width())
    .removeClass('hide');
}
...
action_zimo(zimo, option) {
  const set_gang_handler = (mianzi) => {
    ...
    for (let m of mianzi) {
      let selector
 = `.shoupai.main .shouli .pai[data-pai="${m.substr(0, 2)}"], `
        + `.shoupai.main .shouli .pai[data-pai="${m[0] + m.substr(-1)}"]`;
  ...
  if (this.allow_hule(null, option)) {
    this.set_button('hu', () => this.callback({hule: '-'}));
  }
  ...
  for (let p of this.get_dapai()) {
    let selector = p.substr(-1) == '_'
      ? `.shoupai.main .shouli .zimo .pai[data-pai="${p.substr(0, 2)}"]`
      : `.shoupai.main .shouli > .pai[data-pai="${p}"]`;
    $(selector, this._root).on('click', (e) => {
...
action_dapai(dapai) {
  const set_chi_handler = (mianzi) => {
    ...
    const select_handler = (e, m) => {
      for (let p in map) {
        let selector = `.shoupai.main .shouli .pai[data-pai="${p}"]`;
        $(selector, this._root)
          .off('touchstart')
      ...
      let pair = ($(e.target).data('pai') == s+nn[0])
          ? $(`.shoupai.main .shouli .pai[data-pai="${s+nn[1]}"]`,
              this._root).eq(0)
          : $(`.shoupai.main .shouli .pai[data-pai="${s+nn[0]}"]`,
              this._root).eq(-1);
  ...
  if (this._show_button) this.set_cancel();
